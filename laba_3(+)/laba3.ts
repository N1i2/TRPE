...
const text1 = document.getElementById('numb') as HTMLInputElement;
...
function GetResult(res: string): void {
    console.log(res);
}

function CreatNumber(numb: string): string {
...
const text2 = document.getElementById('ender') as HTMLInputElement;
...
const text3 = document.getElementById('array') as HTMLInputElement;
...
function getResultArray(arr: string, step: number) {
    let first: string = arr.slice(0, arr.length - step);
    let second: string = arr.slice(arr.length - step, arr.length);

    let result: string = second + first;
...
const text41 = document.getElementById('array1') as HTMLInputElement;
const text42 = document.getElementById('array2') as HTMLInputElement;
const but4 = document.getElementById('button4');

let check4 = /^[0-9]+$/;

function GetMedian(arr1: number[], arr2: number[]) {
    let end: number[] = [...new Set([...arr1, ...arr2])];
...
but4?.addEventListener('click', () => {
    if (check4.test(text41.value) && check4.test(text42.value))
        GetMedian(text41.value.split('').map(Number),
            text42.value.split('').map(Number));
    else
